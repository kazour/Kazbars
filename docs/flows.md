...
2. `build_action.build()` — src/kazbars/build_action.py — checks `_building` re-entry guard; validates game folder, compiler path, grids list, total slot count; flags grids that would render empty (no whitelist or no static slot assignments); on the **first** build only (`has_built_before` unset), blocks while an AoC game process is running — the client has to start with the declarations in place for its archive to survive (THE STRIP RULE), and after that `/reloadui` handles the swap
