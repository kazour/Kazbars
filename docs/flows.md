...
3. `get_profile_data()` — Modules/grids_panel.py:926 — calls `save_settings()` then returns `self.grids`
4. `save_settings()` — Modules/grids_panel.py:1009 — iterates all `GridEditorPanel` instances, calling `save_to_config()` on each
5. `save_to_config()` — Modules/grids_panel.py:395 — reads every spinbox/combobox/toggle value and writes it into the grid config dict
...
17. `notify_build_done(use_aoc_bypass)` — Modules/grids_panel.py:864 — re-shows the in-panel tip guide with step 4 marked complete
...
4. `load_profile_data()` — Modules/grids_panel.py:970 — iterates raw grid dicts; migrates, validates, rebuilds panel list; returns `{grid_name: [missing_refs]}` for buffs that couldn't be resolved
5. `_migrate_grid()` — Modules/grids_panel.py:953 — normalizes legacy `int` IDs and legacy name strings in `whitelist` and `slotAssignments` to current primary spell IDs via `database.by_id` and `database.get_entry_by_name`
...
7. `refresh_panels()` — Modules/grids_panel.py:1037 — destroys existing `GridEditorPanel` widgets; creates new ones for the validated list; shows empty state if list is empty
...
4. `get_profile_data()` — Modules/grids_panel.py:926 — calls `save_settings()` then returns `self.grids`
5. `save_settings()` — Modules/grids_panel.py:1009 — iterates all `GridEditorPanel` instances calling `save_to_config()`
6. `save_to_config()` — Modules/grids_panel.py:395 — reads all widget values into the grid config dict
...
Steps:
1. `add_grid()` — Modules/grids_panel.py:1014 — checks the slot budget against `MAX_TOTAL_SLOTS` (64); opens `AddGridWizard` dialog
2. `AddGridWizard.__init__()` — Modules/grid_dialogs.py:43 — builds wizard UI with name, source/mode/dimension fields and four preset shape buttons; calls `restore_window_position()`
3. `AddGridWizard.on_create()` — Modules/grid_dialogs.py:238 — validates name (non-empty, unique, optional special-char warning), enforces slot budget; calls `create_default_grid()`
...
5. `refresh_panels()` — Modules/grids_panel.py:1037 — destroys and recreates all `GridEditorPanel` cards; the newly added card is initially expanded
...
Steps:
1. `_on_mode_btn_click()` — Modules/grids_panel.py:467 — dispatches to `edit_whitelist()` when grid is in dynamic mode (or `edit_slots()` for static)
2. `edit_whitelist()` — Modules/grids_panel.py:473 — flushes current widget state via `save_to_config()`; opens `BuffSelectorDialog`
3. `BuffSelectorDialog.__init__()` — Modules/grid_dialogs.py:284 — resolves initial `whitelist` primary IDs to entry names via `database.by_id`; restores last-used category/type filter from settings; calls `refresh_lists()`
4. `BuffDatabase.search()` — Modules/database_editor.py:81 — filters `grouped_buffs` by query/category/type; sorts by type then name
5. `BuffSelectorDialog.refresh_lists()` — Modules/grid_dialogs.py:381 — repopulates Available and Selected listboxes; selected entries sort by type when the grid `layout` is `buffFirst` or `debuffFirst`, alphabetically when `mixed`
6. `BuffSelectorDialog.on_ok()` — Modules/grid_dialogs.py:448 — saves filter state; maps each selected name back to `entry['ids'][0]` via `database.get_entry_by_name()`; sets `self.result`
7. `update_labels()` — Modules/grids_panel.py:434 — refreshes whitelist count and buff-name preview text in card header
...
7. `GridsPanel.scale_to_resolution()` — Modules/grids_panel.py:1014 — proportionally adjusts each grid's `x`/`y` from `app.reference_resolution` to the selected game resolution; clamps to `SCREEN_MAX_X`/`SCREEN_MAX_Y`; calls `refresh_panels()`
...
Steps:
1. `DatabaseEditorTab.save()` — Modules/database_editor.py:843 — resolves `assets_path / "Database.json"`; calls `BuffDatabase.save()`
...
Steps:
1. `DatabaseEditorTab.add_buff()` — Modules/database_editor.py:693 — creates an add-validator closure (checks ID collision and name uniqueness); opens `BuffEditDialog`
...
5. `DatabaseEditorTab.update_categories()` — Modules/database_editor.py:566 — refreshes the category dropdown values
6. `DatabaseEditorTab.refresh_list()` — Modules/database_editor.py:591 — repopulates treeview rows using current search/category/type filter state; recomputes per-grid usage counts
