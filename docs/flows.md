...
14. `install_to_client(..., damageinfo_swf=..., damageinfo_pristine=..., group_resources=..., source_colors=...)` — src/kazbars/build_executor.py — calls `cleanup_legacy_files()`; copies `KazBars.swf` to `Data/Gui/Default/Flash/`; `_apply_damageinfo()` either atomically installs the staged modded `DamageInfo.swf` (capturing a one-time stock backup at `DamageInfo.swf.kazbars.bak` — seeded from the live file only when it is byte-identical to the bundled pristine stock, otherwise from the bundled pristine itself, so the backup can never capture a mod) or, when `damageinfo_swf` is `None`, restores stock from that backup; `_apply_textcolors()` customizes the skin's `TextColors.xml` (Customized/ if present, else Default/) when `group_resources` (the "Group my resource numbers" toggle) or `source_colors` (per-source color picks) is active — both AND-ed with the master enable in `build_action` — by keeping a one-time genuine-stock backup and **regenerating** the live file from it (stock → resource-loss direction flip → per-source color overrides), else restoring stock from that backup; calls `write_xml_add_files()` in Aoc mode; calls `create_scripts()`
...
End state: `KazBars.swf` installed under the game folder; `Scripts/reloadgrids` and `Scripts/unloadgrids` written; in non-Aoc mode `Scripts/auto_login` updated; in Aoc mode `Data/Gui/Aoc/KazBars/MainPrefs.xml.add` and `Modules.xml.add` written; when Damage Numbers is enabled the modded `DamageInfo.swf` is installed (stock backed up to `DamageInfo.swf.kazbars.bak`), and when disabled any prior mod is reverted from that backup; with "Group my resource numbers" on and/or per-source colors set, the skin's `TextColors.xml` is regenerated from a one-time stock backup with the resource-loss directions flipped and/or the chosen `color="0x…"` per source (restored to stock when neither is active); build loading screen shows the result summary
...
3. `DamageNumbersPanel.__init__()` — src/kazbars/damageinfo_panel.py — `restore_window_position("damage_numbers", …)`; loads `damageinfo_settings.json` via `load_settings()`; builds the UI (CRT header, tip bar, master Enable checkbutton, then a scrollable body — Presets, a "Damage number colors…" launch button, then cards Shadow / Size / Above-target position / Fixed columns / Zig-zag / Behavior — built from `damageinfo_settings.GLOBAL_SETTINGS` metadata); `_sync_enabled_state()` greys every control (incl. the colors button) when the master toggle is off; `WM_DELETE_WINDOW` → destroy.
...
Trigger: In the Damage Numbers panel, the user clicks "Damage number colors…".

Steps:
1. `DamageNumbersPanel._build_colors_button` command → `open_damage_number_colors_panel(self.master)` — src/kazbars/damageinfo_colors_panel.py — single-instance gate (mirrors Flow 20); constructs `DamageNumberColorsPanel(app, app.settings_path, app.game_path)`.
2. `DamageNumberColorsPanel.__init__()` — loads `damageinfo_settings.json`; `_read_baseline_colors(game_path)` reads the skin's `TextColors.xml` (the `.kazbars.bak` stock backup if present, else the live Customized/Default file via `buff_xml._resolve_paths`) into `name → RRGGBB`; builds a 2-column body — each `damageinfo_settings.PAIRED_GROUPS` entry as a Self card (left) + Other card (right), then a full-width "Resources & misc" card from `SHARED_SOURCES`. Each row = label + `ui_forms.ColorSwatch` (initial = saved override else baseline) + a "↺" reset.
...
End state: `damageinfo_settings.json`'s `source_colors` reflects the picks. Applied in-game only on the next Build & Install (Flow 1, step 14), gated by the master enable — `_apply_textcolors` writes each `color="0x…"` into `TextColors.xml`; disabling/uninstalling restores stock from the backup.
