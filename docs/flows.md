...
4. `_apply()` — src/kazbars/inspect_panel.py — re-validates via `inspect.validate_config` (clamps X/Y + fontSize, coerces the collapse and section flags, falls back to the loaded value on an emptied spinbox), sets the flat `build_console` key, writes the dict to `prefs['inspect']`, saves both with one `save()`, resyncs the main-screen extras card (`refresh_extras_shortcuts()`), toasts, and destroys the dialog. The toast has three branches: when the validated dict is unchanged the only thing that can have moved is the console checkbox, so it says "Console saved — Build & Install to apply"; otherwise "Inspect panel saved — Build & Install to apply" when enabled / "Inspect panel off — next build removes it" when disabled.

End state: prefs.json's `inspect` dict and the flat `build_console` key reflect the choices — the console then compiles into the next build via `include_console` (Flow 1, step 9). Nothing is applied in-game yet — on the next Build & Install (Flow 1, steps 9–11) `include_inspect` compiles the `KazBarsInspect` stub into the module: a combat sheet for the current target in the visual language of the game's default inspect window, fed by a 250 ms stat poll with a ~750 ms warm-up so retargets never flash stale values, and pointed at the target from `SlotTargetChanged`. It shows the PvE mitigation/crit sheet for any target (destructibles included) and, on player targets only (classified by `ID32.IsPlayer()`), adds the PvP section and the Perks row — the target's slotted-AA perk buffs as a row of game icons — each behind its baked section toggle. Every dimension derives from the baked font size, so the panel scales as one piece. Dragging the name strip shows live coordinates — `/loadclip` users copy them into this dialog to make a spot permanent; the strip's − / + button folds the panel to a small bar reading just `Inspect` (`fontSize × 15.8` by `fontSize × 2` — the stopwatch's collapsed bar comes off the same ratios, so the two match at any size), which re-opens the sheet when pressed and costs a two-id poll while it is folded; drag position and collapsed state persist automatically via the module config archive (`inx`/`iny`/`inc`), reading the fold flag first so the position clamp measures the plate actually on screen.
