...
2. `DamageNumberColorsPanel.__init__()` — resolves the skin's `TextColors.xml` via `buff_xml._resolve_paths` (Customized if present, else Default); `_read_colors(source)` / `_read_directions(source)` seed the rows from the live file and `_read_colors(default)` / `_read_directions(default)` capture the stock color + direction each "reset" reverts to. Builds a 2-column body — a full-width "Number directions" macro card on top, then each `damageinfo_settings.PAIRED_GROUPS` entry as a Self card (left) + Other card (right), then a full-width "Resources & misc" card from `SHARED_SOURCES`. Each row (`_build_source_row`) reads label + a Rising/Dropping/Zig-zag direction combobox (1 / −1 / 0) + `ui_forms.ColorSwatch` + a "↺" reset, so the reset sits past both of the things it resets. A bottom row packs Cancel + Apply (Apply disabled until a pick differs from the loaded baseline); the panel snapshots that baseline after building and calls `_refresh_macros()`.
3. User picks a color → `_on_color(name, hex)` records the pick; picks a direction → `_on_direction(name)` records it and re-runs `_refresh_macros()`; either enables Apply via `_refresh_apply_state()`. "↺" → `_reset_one(name)` restores that row's stock color *and* direction (`_restore_stock`); "Reset all to game default" → `_reset_all()` does so for every row. None of these touch disk — they stage pending edits (Cancel/Escape abandons them).
4. User ticks a macro checkbox → `_on_macro(names, var)` — src/kazbars/damageinfo_colors_panel.py — stages a whole group's rows to Dropping (−1) when checked, Rising (1) when unchecked, moving the visible comboboxes with them (`_set_direction`): "Group my resource numbers" drives `buff_xml.RESOURCE_LOSS_TYPES`, "Send incoming numbers to the fixed column" drives `INCOMING_DAMAGE_TYPES`. The checkboxes are derived state — `_refresh_macros()` re-ticks each one only while every row in its group reads −1, so hand-tweaking one row unticks it.
5. User clicks Apply → `_on_apply()` — src/kazbars/damageinfo_colors_panel.py — sends every color pick but only the directions that differ from the baseline (a direction write *injects* the attribute when the source omits it, so re-writing untouched rows would rewrite ~35 elements), calling the module's pure `apply_colors(game_path, picks, directions)`: reads the file the game reads (Customized else Default), applies each pick via `buff_xml.set_source_color` and each direction via `buff_xml.set_source_direction` (both element-scoped and skip-when-equal), then writes to **Customized/TextColors.xml** atomically via `safe_write_text` (created from the Default copy when absent), taking a one-time `TextColors.xml.kazbars.bak` of any pre-existing skin file first. On success the panel re-baselines (Apply disables) and toasts "type /reloadui"; an `OSError` (game holding the file locked, permissions) toasts a failure and changes nothing.

End state: colors and directions are written straight into `Customized/TextColors.xml` — no build, no master-enable gate (mirrors the Default Buff Bars editor, Flow 15). The user types `/reloadui` in-game to see them. This panel is the file's only writer: Build & Install (Flow 1) and Uninstall (Flow 9) never touch it, so a chosen direction survives a rebuild, a mod-off build and an uninstall alike — "Reset to game default" is the way back to stock.
