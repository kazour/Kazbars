...
4. `_apply()` — src/kazbars/cast_timer_panel.py — re-validates via `cast_timer.validate_config` (clamps X/Y and fontSize, sanitizes the colour and the display enum, falls back to the loaded value on an emptied spinbox), sets `enabled`/`enableP`/`enableT` all to the one master toggle (the schema keeps the per-side flags for the generator contract; the dialog has never split them), writes the dict to `prefs['cast_timer']`, saves, resyncs the main-screen extras card (`refresh_extras_shortcuts()`), toasts ("Cast timer saved — Build & Install to apply" when enabled / "Cast timer off — next build removes it" when disabled), and destroys the dialog.

End state: prefs.json's `cast_timer` dict reflects the choices — machine-local like `stopwatch` and `inspect`, since the X/Y depend on the screen rather than the buff layout, so switching or sharing a profile leaves the timer alone. Nothing is applied in-game yet — on the next Build & Install (Flow 1, steps 9–11) `include_cast_timer` (via `cast_timer.is_enabled`: master on **and** at least one side on) compiles the `KazBarsCastTimer` stub into the module and bakes the config into `KazBarsData`'s `d.CAST` block: a timer-only overlay (no bar) reading player and target cast time, drawn at the baked positions. Shift+Ctrl+Alt toggles preview mode in-game so a timer can be dragged with its coordinates showing, ready to copy back into this dialog; the drag itself persists via the module config archive (`ctpx`/`ctpy` for the player timer, `cttx`/`ctty` for the target one) — for every user, since Flow 1 declares the module permanently; the baked positions are just where a first-ever session starts.
