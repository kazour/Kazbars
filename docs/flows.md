...
End state: `KazBars.swf` installed under the game folder; `Scripts/reloadgrids` and `Scripts/unloadgrids` written; in non-Aoc mode `Scripts/auto_login` updated; in Aoc mode `Data/Gui/Aoc/KazBars/MainPrefs.xml.add` and `Modules.xml.add` written; when Damage Numbers is enabled the modded `DamageInfo.swf` is installed (stock backed up to `DamageInfo.swf.kazbars.bak`), and when disabled any prior mod is reverted from that backup; with "Group my resource numbers" / "Split into two columns" on and/or per-source colors set, the skin's `TextColors.xml` is regenerated from a one-time stock backup with the resource-loss and/or incoming/self directions flipped and/or the chosen `color="0x…"` per source (restored to stock when none is active); build loading screen shows the result summary
...
3. `DamageNumbersPanel.__init__()` — src/kazbars/damageinfo_panel.py — `restore_window_position("damage_numbers", …)`; loads `damageinfo_settings.json` via `load_settings()`; builds the UI (CRT header, tip bar, master Enable checkbutton, then a scrollable body — a "Damage number colors…" launch button, Presets, then cards Behavior (all toggles, off by default) / Shadow / Above-target position / Fixed columns / Zig-zag — built from `damageinfo_settings.GLOBAL_SETTINGS` metadata; no number/label size slider, AoC's own Options slider covers it); `_sync_enabled_state()` greys every control (incl. the colors button) when the master toggle is off; `WM_DELETE_WINDOW` → destroy.
