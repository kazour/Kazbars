...
5. `profile_io.apply_document()` — src/kazbars/profile_io.py — the dispatch step: grids into the panel, missing-ref warning, boss timer + Deeps (when alive), extras shortcut cards (`refresh_extras_shortcuts()` — the stopwatch/inspect/cast-timer gates travel with the profile), `active_profile` pointer, title, File-menu refresh. At startup it must run before `deiconify()` (see step 9's deferral)
