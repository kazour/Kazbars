...
15. `splice_declarations(game_path, discover_aoc_archive_declarations(game_path))` — src/kazbars/game_persistence.py — inserts a `<!-- KazBars begin/end -->` block before `</Root>` in `Data/Gui/Default/MainPrefs.xml` (the `KazBars` value + `KazBars settings` archive) and in `modules_target()` — `Data/Gui/Customized/Modules.xml` when that file exists, else `Data/Gui/Default/Modules.xml`. Idempotent: an existing block is refreshed in place. Seeds a one-time `*.kazbars.bak` per XML before the first splice, and writes byte-faithfully (the XMLs are LF-only and the patcher hashes them)
