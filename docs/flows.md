...
14. `install_to_client(..., damageinfo_swf=...)` — src/kazbars/build_executor.py — calls `cleanup_legacy_files()`; copies `KazBars.swf` to `Data/Gui/Default/Flash/`; `_apply_damageinfo()` either installs the staged modded `DamageInfo.swf` (backing the stock file up once to `DamageInfo.swf.kazbars.bak`) or, when `damageinfo_swf` is `None`, restores stock from that backup; calls `write_xml_add_files()` in Aoc mode; calls `create_scripts()`
...
End state: `KazBars.swf` installed under the game folder; `Scripts/reloadgrids` and `Scripts/unloadgrids` written; in non-Aoc mode `Scripts/auto_login` updated; in Aoc mode `Data/Gui/Aoc/KazBars/MainPrefs.xml.add` and `kazbars.xml.add` written; when Damage Numbers is enabled the modded `DamageInfo.swf` is installed (stock backed up to `DamageInfo.swf.kazbars.bak`), and when disabled any prior mod is reverted from that backup; build loading screen shows the result summary
