...
6. User clicks Apply → `_on_apply()` — src/kazbars/damageinfo_panel.py — the single write: `damageinfo_settings.save_settings()` persists the staged dict, a success toast points at Build & Install, and the panel `destroy()`s. Cancel, `<Escape>` or the X destroy without saving, discarding every staged edit.
...
2. `DamageNumberColorsPanel.__init__()` — resolves the skin's `TextColors.xml` via `buff_xml._resolve_paths` (Customized if present, else Default); `_read_colors(source)` / `_read_directions(source)` seed the rows from the live file and `_read_colors(default)` / `_read_directions(default)` capture the stock color + direction each "reset" reverts to. Builds a 2-column body — a full-width "Number directions" macro card on top, then each `damageinfo_settings.PAIRED_GROUPS` entry as a Self card (left) + Other card (right), then a full-width "Resources & misc" card from `SHARED_SOURCES`. Each row = label + a Rising/Dropping/Zig-zag direction combobox (1 / −1 / 0) + a "↺" reset + `ui_forms.ColorSwatch`. A bottom row packs Cancel + Apply (Apply disabled until a pick differs from the loaded baseline); the panel snapshots that baseline after building and calls `_refresh_macros()`.
