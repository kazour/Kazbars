...
2. `open_inspect_dialog(app)` — src/kazbars/inspect_panel.py — single-instance gate on `app.inspect_dialog` (deiconify + lift + focus if open); otherwise builds a modal Toplevel (`transient` + `grab_set`) over the validated `inspect` prefs dict, `withdraw → build → restore_window_position('inspect_settings', …, offset=(30, 30)) → bind_window_position_save(save_size=False) → deiconify` so it reopens where the user left it (staggered off the stopwatch dialog's first-launch centre): CRT header, master "Include the inspect panel in builds" checkbutton, X/Y spinboxes for the baked default position, a **Text size** section — an "All panels" spinbox (8–48) carrying the flat `panel_font_size` all four in-game panels share, plus a "Use the shared size for this panel" checkbutton over an "Inspect panel only" override spinbox that it disables — a start-collapsed checkbutton, and the two section checkbuttons — "Show the PvP section" and "Track slotted perks" (both default on). Binds `<Escape>` → destroy, re-asserts the dark titlebar on `<Map>`, and puts initial focus on the enable checkbutton via `after(0, …)`.
3. A "Console" section at the bottom carries "Include the buff-discovery console in builds", seeded from the flat `build_console` pref — the other SWF-side inspection tool, switched on from the same dialog rather than the Extras menu.
