...
5. `profile_io.apply_document()` — src/kazbars/profile_io.py — the dispatch step: grids into the panel, boss timer + Deeps (when alive), extras shortcut cards (`refresh_extras_shortcuts()` — the stopwatch/inspect/cast-timer gates travel with the profile), `active_profile` pointer, title, File-menu refresh. At startup it must run before `deiconify()` so panels are populated before the window shows
...
7. `app.settings.set('game_resolution', [new_w, new_h])` + save, then `refresh_panels()` (src/kazbars/grids_panel.py) rebuilds the cards so every pixel field re-projects the stored fractions at the new size. Nothing in the document moves — positions are fractions
