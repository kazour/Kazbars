...
Trigger: User selects Extras ▸ Target inspect panel… from the menu.

Steps:
1. `KazBarsApp._open_inspect_settings()` — src/kazbars/app.py — one-line delegator to `open_inspect_dialog(self)`.
2. `open_inspect_dialog(app)` — src/kazbars/inspect_panel.py — single-instance gate on `app.inspect_dialog` (lift + focus if open); otherwise builds a modal Toplevel over the validated `inspect` prefs dict: CRT header, master "Include the inspect panel in builds" checkbutton, X/Y spinboxes for the baked default position, a font-size spinbox (8–48), a start-collapsed checkbutton, and the two section checkbuttons — "Show the PvP section" and "Track slotted perks" (both default on).
3. `_apply()` — src/kazbars/inspect_panel.py — re-validates via `inspect.validate_config` (clamps X/Y + fontSize, coerces the collapse and section flags, falls back to the loaded value on an emptied spinbox), writes the dict to `prefs['inspect']`, saves, toasts ("Build & Install to apply" when enabled / "next build removes it" when disabled), and destroys the dialog.

End state: prefs.json's `inspect` dict reflects the choices. Nothing is applied in-game yet — on the next Build & Install (Flow 1, steps 9–11) `include_inspect` compiles the `KazBarsInspect` stub into the module: a combat sheet for the current target in the visual language of the game's default inspect window, fed by a 250 ms stat poll with a ~750 ms warm-up so retargets never flash stale values, and pointed at the target from `SlotTargetChanged`. It shows the PvE mitigation/crit sheet for any target (destructibles included) and, on player targets only (classified by `ID32.IsPlayer()`), adds the PvP section and the Perks row — the target's slotted-AA perk buffs as a row of game icons — each behind its baked section toggle. Every dimension derives from the baked font size, so the panel scales as one piece. Dragging the name strip shows live coordinates — `/loadclip` users copy them into this dialog to make a spot permanent; the strip's − / + button folds the panel to a small bar reading just `Inspect`, sized to the stopwatch's own collapsed bar, which re-opens the sheet when pressed and costs a two-id poll while it is folded; aoc.exe clients persist drag position and collapsed state automatically via the module config archive (`inx`/`iny`/`inc`).
