...
5. `profile_io.apply_document()` — src/kazbars/profile_io.py — the dispatch step: grids into the panel, missing-ref warning, boss timer (when alive), extras shortcut cards (`refresh_extras_shortcuts()` — the stopwatch/inspect/cast-timer gates travel with the profile), `active_profile` pointer, title, File-menu refresh. At startup it must run before `deiconify()` (see step 9's deferral)
...
11. `app.settings.set('active_profile', ...)` then `app.settings.save()` — persists the id pointer to `userdata/prefs.json` (atomic temp-rename in `safe_save_json`). No resolution rescale exists: positions are stored as fractions (`fx`/`fy`), projected to px only at display and build time, so the document is never rewritten by a resolution difference — `authored_at` is display-only provenance
...
2. `_on_toggle()` — src/kazbars/extras_shortcuts.py — reads the flipped var and calls the tile's writer with the new state: the exact write its Extras dialog makes. Stopwatch/inspect flip `enabled` in their profile section via `ProfileStore.update_section` (debounced autosave, Flow 3); the cast timer flips `enabled`/`enableP`/`enableT` together (`is_enabled` needs master AND a side); damage numbers round-trips `damageinfo_settings.json` via `load_settings`/`save_settings` — lossless for declared keys. No store race is possible: all four Extras dialogs are modal, so none can be open while a card is clickable.
3. `refresh()` — src/kazbars/extras_shortcuts.py — resyncs every tile from its store, no-op for unchanged ones: BooleanVar, card border (`success` when in the next build, neutral `border` when off — the resting colour `bind_card_events` hover restores to), and the status line ("In next build" in success / "Off" in dim). The three profile-backed reads rest off while `app.profile_store` is still `None` (the row is built with `GridsPanel`, before `startup_profile` installs the store). The same method is the push target the four dialogs call after their own Apply, via `GridsPanel.refresh_extras_shortcuts()` — src/kazbars/grids_panel.py — and the target `profile_io.apply_document` hits on every profile switch, since the three gates travel with the profile.
