...
14. `install_to_client(..., damageinfo_swf=..., damageinfo_pristine=..., group_resources=...)` — src/kazbars/build_executor.py — calls `cleanup_legacy_files()`; copies `KazBars.swf` to `Data/Gui/Default/Flash/`; `_apply_damageinfo()` either atomically installs the staged modded `DamageInfo.swf` (capturing a one-time stock backup at `DamageInfo.swf.kazbars.bak` — seeded from the live file only when it is byte-identical to the bundled pristine stock, otherwise from the bundled pristine itself, so the backup can never capture a mod) or, when `damageinfo_swf` is `None`, restores stock from that backup; `_apply_textcolors()` flips the four resource-loss flytext directions in the skin's `TextColors.xml` (Customized/ if present, else Default/) to `-1` when `group_resources` (the "Group my resource numbers" toggle, AND-ed with the master enable in `build_action`) else restores `1` — surgical + reversible, one-time courtesy backup; calls `write_xml_add_files()` in Aoc mode; calls `create_scripts()`
...
End state: `KazBars.swf` installed under the game folder; `Scripts/reloadgrids` and `Scripts/unloadgrids` written; in non-Aoc mode `Scripts/auto_login` updated; in Aoc mode `Data/Gui/Aoc/KazBars/MainPrefs.xml.add` and `Modules.xml.add` written; when Damage Numbers is enabled the modded `DamageInfo.swf` is installed (stock backed up to `DamageInfo.swf.kazbars.bak`), and when disabled any prior mod is reverted from that backup; with "Group my resource numbers" on, the skin's `TextColors.xml` has its four resource-loss directions flipped to the fixed column (restored to stock when off); build loading screen shows the result summary
