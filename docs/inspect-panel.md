...
- **Subject resolution.** `Character.GetCharacter(tid)` first, falling back to `Dynel.GetDynel(tid)` so destructibles and simple dynels still read (stats yes, buffs no). This is why a minimal `Dynel` intrinsic sits in `assets/common_stubs/com/GameInterface/Game/`. Identity key is `GetID().GetType() + ":" + GetInstance()`; re-targeting the same entity keeps the warm cache instead of restarting the warm-up.
