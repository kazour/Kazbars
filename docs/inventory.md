# File inventory

**Purpose:** one row per `src/kazbars/*.py` and `tests/*.py` file — path, line count. Generated by `scripts/gen_inventory.py`; re-run on any tracked `.py` change via pre-commit. Per-file role and behavior notes live in [`inventory-roles.md`](inventory-roles.md). Machine-checked by `tests/test_docs_in_sync.py`: no phantoms, no omissions, exact line counts. Module topology and clusters live in [`architecture.md`](architecture.md).

> Never add another table row shaped `| src/kazbars/x.py | <number> | …` to any other
> doc — the row regex scans every line of this file only, but a stray match elsewhere
> would go unnoticed. This table is generated; edit `scripts/gen_inventory.py`, not
> the rows.

## Files

| File | Lines |
|---|---:|
| `src/kazbars/__init__.py` | 11 |
| `src/kazbars/__main__.py` | 43 |
| `src/kazbars/app.py` | 716 |
| `src/kazbars/app_popups.py` | 489 |
| `src/kazbars/boss_timer.py` | 399 |
| `src/kazbars/buff_database.py` | 176 |
| `src/kazbars/buff_db_layers.py` | 267 |
| `src/kazbars/buff_display_editor.py` | 592 |
| `src/kazbars/buff_xml.py` | 313 |
| `src/kazbars/build_action.py` | 294 |
| `src/kazbars/build_executor.py` | 467 |
| `src/kazbars/build_loading.py` | 491 |
| `src/kazbars/build_utils.py` | 106 |
| `src/kazbars/cast_timer.py` | 89 |
| `src/kazbars/cast_timer_strip.py` | 344 |
| `src/kazbars/combat_monitor.py` | 301 |
| `src/kazbars/content_update.py` | 400 |
| `src/kazbars/custom_menu_bar.py` | 436 |
| `src/kazbars/damageinfo_colors_panel.py` | 283 |
| `src/kazbars/damageinfo_generator.py` | 134 |
| `src/kazbars/damageinfo_panel.py` | 410 |
| `src/kazbars/damageinfo_settings.py` | 483 |
| `src/kazbars/database_editor.py` | 850 |
| `src/kazbars/deeps_meter.py` | 485 |
| `src/kazbars/deeps_overlay.py` | 738 |
| `src/kazbars/deeps_panel.py` | 933 |
| `src/kazbars/deeps_parsers.py` | 408 |
| `src/kazbars/deeps_rolling_window.py` | 81 |
| `src/kazbars/deeps_settings.py` | 354 |
| `src/kazbars/deeps_trackers.py` | 182 |
| `src/kazbars/first_launch.py` | 384 |
| `src/kazbars/focus_watcher.py` | 85 |
| `src/kazbars/foreground.py` | 111 |
| `src/kazbars/game_folder.py` | 180 |
| `src/kazbars/game_resolution.py` | 105 |
| `src/kazbars/grid_dialogs.py` | 865 |
| `src/kazbars/grid_editor_panel.py` | 635 |
| `src/kazbars/grid_model.py` | 192 |
| `src/kazbars/grids_generator.py` | 696 |
| `src/kazbars/grids_panel.py` | 683 |
| `src/kazbars/inspect.py` | 52 |
| `src/kazbars/inspect_panel.py` | 169 |
| `src/kazbars/instructions_panel.py` | 913 |
| `src/kazbars/live_tracker_panel.py` | 511 |
| `src/kazbars/live_tracker_settings.py` | 183 |
| `src/kazbars/overlay_engine.py` | 800 |
| `src/kazbars/paths.py` | 46 |
| `src/kazbars/prefs.py` | 103 |
| `src/kazbars/profile_io.py` | 270 |
| `src/kazbars/profile_manager.py` | 323 |
| `src/kazbars/profile_share.py` | 137 |
| `src/kazbars/settings_backup.py` | 472 |
| `src/kazbars/settings_core.py` | 255 |
| `src/kazbars/settings_manager.py` | 56 |
| `src/kazbars/stopwatch.py` | 47 |
| `src/kazbars/stopwatch_panel.py` | 132 |
| `src/kazbars/timer_overlay.py` | 372 |
| `src/kazbars/ui_collapsible.py` | 232 |
| `src/kazbars/ui_components.py` | 624 |
| `src/kazbars/ui_forms.py` | 433 |
| `src/kazbars/ui_headers.py` | 197 |
| `src/kazbars/ui_helpers.py` | 195 |
| `src/kazbars/ui_tk_style.py` | 67 |
| `src/kazbars/ui_widgets.py` | 320 |
| `src/kazbars/update_check.py` | 114 |
| `src/kazbars/userdata.py` | 86 |
| `src/kazbars/window_position.py` | 116 |
| `tests/test_app_contract.py` | 104 |
| `tests/test_as2_class_names.py` | 61 |
| `tests/test_boss_timer.py` | 151 |
| `tests/test_buff_db_layers.py` | 266 |
| `tests/test_buff_xml.py` | 274 |
| `tests/test_build_compile.py` | 137 |
| `tests/test_build_executor.py` | 617 |
| `tests/test_cast_timer.py` | 97 |
| `tests/test_cluster_isolation.py` | 184 |
| `tests/test_combat_monitor.py` | 150 |
| `tests/test_content_update.py` | 305 |
| `tests/test_damageinfo_colors_panel.py` | 104 |
| `tests/test_damageinfo_generator.py` | 183 |
| `tests/test_damageinfo_settings.py` | 373 |
| `tests/test_data_integrity.py` | 115 |
| `tests/test_deeps_meter.py` | 469 |
| `tests/test_deeps_overlay.py` | 374 |
| `tests/test_deeps_parsers.py` | 544 |
| `tests/test_deeps_rolling_window.py` | 169 |
| `tests/test_deeps_settings.py` | 517 |
| `tests/test_deeps_trackers.py` | 271 |
| `tests/test_design_tokens.py` | 91 |
| `tests/test_docs_in_sync.py` | 301 |
| `tests/test_focus_watcher.py` | 96 |
| `tests/test_foreground.py` | 95 |
| `tests/test_grid_model.py` | 79 |
| `tests/test_grids_generator.py` | 316 |
| `tests/test_hud_overlay.py` | 141 |
| `tests/test_imports.py` | 33 |
| `tests/test_inspect.py` | 73 |
| `tests/test_log_name.py` | 22 |
| `tests/test_manifest.py` | 59 |
| `tests/test_mypy_gate.py` | 74 |
| `tests/test_overlay_config.py` | 102 |
| `tests/test_panels_construct.py` | 247 |
| `tests/test_prefs_schema_covers_all_proxy_keys.py` | 83 |
| `tests/test_profile_io.py` | 64 |
| `tests/test_profile_share.py` | 152 |
| `tests/test_resolution_scaling.py` | 71 |
| `tests/test_settings_backup.py` | 159 |
| `tests/test_settings_core.py` | 310 |
| `tests/test_settings_manager.py` | 33 |
| `tests/test_stopwatch.py` | 52 |
| `tests/test_subprocess_flags.py` | 50 |
| `tests/test_timer_sizing.py` | 41 |
| `tests/test_toast_model.py` | 213 |
| `tests/test_toggle_button_state.py` | 41 |
| `tests/test_update_check.py` | 35 |
| `tests/test_userdata.py` | 68 |
