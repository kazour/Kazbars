...
- **AS2 internals stay `KzGrids`.** The precompiled `base.swf` hardcodes the old class names, so the ActionScript classes and stubs keep the `KzGrids` prefix while everything game-side renames to `KazBars`. (Lifting this would need a Flash IDE recompile of `base.fla`.)
