...
Bodies of the pre-2.2.0 releases, moved out of [`CHANGELOG.md`](CHANGELOG.md) on 2026-07-30 so the live file stays cheap to read whole. Nothing here changes again — it is the shipped record of v2.1.0, v2.0.0 and v1.1.0.

The live changelog keeps a stub heading for each of these versions: `tests/test_docs_in_sync.py` asserts every `v*` release tag has a `## [X.Y.Z]` section there, so the stubs must never be deleted.
