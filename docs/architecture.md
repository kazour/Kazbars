...
**Current as of:** 2026-08-18 (AS2 restructure: `KazBars.as.template`'s optional features are `//#if CONSOLE|CAST|SW|INS … //#end` blocks kept or stripped by `grids_generator._apply_feature_gates()` — no `{{…}}` placeholders remain; console persistence moved wholly into `KazBarsConsole.loadState`/`saveState`, and the chrome-family base `KazBarsPanel` owns the family font-size clamp in `applyBaseSize` plus the `hairline`/`makeCollapsedLabel`/`makeCoordReadout` primitives).
