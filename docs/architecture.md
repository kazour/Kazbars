...
| `src/kazbars/damageinfo_settings.py` | 413 | Damage Numbers config (pure data): `GLOBAL_SETTINGS` bake-map (symmetric offset ranges + `invert`/`relative` UI flags for the position sliders + target file + regex), `GAME_DEFAULTS`, `PRESETS` (Default/Performance — carry the animation timing), `SPREAD_SPACING_OPTIONS` (one radio → both zig-zag offsets) + `spread_spacing_option`, validate/`compute_final_value`/`readout` (signed-shift vs game-value label)/`apply_preset`, `is_offset_key` (offset vs absolute → centre-notch), load/save. No Tk |
...
| `src/kazbars/damageinfo_panel.py` | 359 | `DamageNumbersPanel` Toplevel (Game ▸ Damage numbers…) — master enable gate, presets (Default/Performance, which carry animation timing), offset sliders (centre-notched; vertical ones reversed so right = up; position sliders read a signed shift via `dis.readout`), the coupled `Spread-spacing` radio (one control → both zig-zag offsets), radios/checkboxes across Distance/Shadow/Size/Position/Behavior cards in a scrollable body; persists to `damageinfo_settings.json` |
...
| `tests/test_damageinfo_settings.py` | 310 | Damage Numbers config — defaults/schema invariants, symmetric offset ranges + common X/Y step + `is_offset_key` + `invert`/`relative` position-slider sets, offset clamping, enum/bool coercion, `compute_final_value`, `readout` (signed-shift vs game-value), `apply_preset` (Default/Performance only), round-trip + corrupt/partial-file fallback |
| `tests/test_damageinfo_generator.py` | 185 | Regex↔AS2 coupling guard (every bake pattern matches the shipped source **exactly once**) + shipped-constant == GAME_DEFAULTS (the offset-0-is-stock invariant) + bake correctness (offset→final, dual-axis shadow blur, enum/bool) + per-content-scale guard (Size survives the pop-in) + easing-ships-Quad guard + hard-fail on drifted source; no MTASC |