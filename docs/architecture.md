...
|  2 | `prefs`, `grids_generator`, `update_check` | `prefs`: `app.py` (builds the `Store` on `PREFS_SCHEMA`) + `grids_generator` (`validate_panel_font_size` only — the shared panel size is a prefs-level contract, and the generator is what bakes it). `update_check`: `app.py` (launch check) + `app_popups` (About ▸ Check for updates via `fetch_latest`). |
