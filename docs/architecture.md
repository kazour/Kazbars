...
**Current as of:** 2026-08-29 (v3.0.0 doc sync — the profile-system rewrite is shipped, not in-progress; `profile_io`/`profile_library`/`content_update`/`game_persistence` fan-in recounted for the new `settings_backup`/`build_action` call sites).
...
```
content_update  ← app.py, first_launch, profile_io
  → buff_db_layers (summarize), userdata (content_dir), ui_widgets (toast)
update_check    (GitHub release check; content_update borrows its _parts version parser)
```
...
|  3 | `build_executor`, `game_persistence`, `live_tracker_settings`, `paths`, `stopwatch`, `inspect`, `update_check` | Cluster leaves. `update_check` ← `app.py` (launch check), `app_popups` (About ▸ Check for updates via `fetch_latest`), `content_update` (`_parts` version parser). `paths` is imported directly by `app.py`, `build_utils`, `deeps_parsers` (everyone else gets paths via the `app` object). `game_persistence` ← `build_executor`, `build_action`, `game_folder` — the game-folder layer of the build cluster; changing a marker, a target path or a constant there is felt by all three. `stopwatch` and `inspect` are the pure config layers ← `grids_generator`, `app.py` (registers their `PROFILE_SECTION`s), and their own Extras dialog (`stopwatch_panel` / `inspect_panel`). Both their `fontSize` fields are nullable — `None` defers to `prefs`'s flat `panel_font_size`, which `grids_generator` resolves and bakes. |
|  2 | `grids_generator` | Build-cluster leaf. |
