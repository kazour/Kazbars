...
| `src/kazbars/app.py` | 676 | Entry point + `KazBarsApp` root window (widgets, menu, lifecycle); `__init__` calls `ensure_layout()`, points `profiles_path`/`settings_path` at `userdata/`, builds `self.settings = Prefs(userdata_root())`, loads the buff DB via `database.load_layers(stock, content, user)` (DB editor wired with a `DeltaStore` + `get_floor`), and kicks the OTA check (`content_update.check_and_apply`, deferred to first-launch completion on a fresh install); Game menu carries the auto-update toggle + check-now/revert |
