...
**Current as of:** 2026-08-17 (P8: `damage_numbers` — BUILD lane — and `deeps` — LIVE lane — join the profile document as flat, disk-less `PROFILE_SECTION`s, mirroring `boss_timer`; `userdata/settings/` is retired — `ensure_layout()` no longer creates it, `settings_backup`'s allowlist drops it, and a pre-P8 backup's `settings/` entries are skipped on restore rather than resurrecting the directory; `DeepsPanel`/`LiveTrackerPanel`/`DamageNumbersPanel` seed from and write through `app.profile_store` instead of a `settings_path` constructor arg; `DeepsOverlay.apply_settings()` (mirrors `TimerOverlay.apply_settings()`) lets a profile switch restyle an open Deeps overlay live via a new `_deeps_panel_if_alive()`/`on_deeps_profile_data()` seam in app.py).
...
profile_document ← grid_model + live_tracker_settings + deeps_settings + stopwatch
                   + inspect + cast_timer + damageinfo_settings (each exports PROFILE_SECTION)
...
| 10 | `profile_document` | The profile section contract ← `app.py` (registry wiring), `grid_model` + `live_tracker_settings` + `deeps_settings` + `stopwatch` + `inspect` + `cast_timer` + `damageinfo_settings` (each exports a `PROFILE_SECTION`), `profile_library` (the gate), `profile_io`. Infrastructure on the cluster-isolation list; imports only `settings_core`. |
