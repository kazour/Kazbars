...
**Current as of:** 2026-08-17 (the stopwatch/inspect/cast-timer configs moved from prefs.json into profile-document sections — `stopwatch`/`inspect`/`cast_timer`, BUILD lane, fraction positions from birth; their dialogs and the extras shortcut row read/write the profile store, and `apply_document` resyncs the cards on every switch. `panel_font_size` and `build_console` stay machine-local prefs).
...
<install>/
  ...
  userdata/                       ← USER + MACHINE (created fresh by ensure_layout() on first launch)
    prefs.json                    ← machine-local (window positions, game path, resolution, the `active_profile` pointer, build toggles incl. `build_console`, the flat `panel_font_size` the four in-game panels share, UI state)
