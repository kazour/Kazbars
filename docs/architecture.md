...
**Current as of:** 2026-07-13 (monitor restart-race guards + atomic `safe_write_text` skin writes — one new import edge `damageinfo_colors_panel` → `settings_manager`, two new test files, counts refreshed).
...
| `tests/test_panels_construct.py` | 246 | Panel-construction smoke — boots a real `KazBarsApp` (tmp userdata, network no-opped, prefs pre-seeded) and opens all 8 satellite panels via their app-side openers, closing each via `WM_DELETE_WINDOW`; AST canary self-enrolls new `_open_*`/`_show_*` openers (or an explicit DEFERRED entry). Also hosts the ui_widgets binding-hygiene tests (idempotent `add_tooltip`, hover-colors add="+") — they must ride this module's Tk instance, since a second Tcl interp won't initialize on the hosted CI runner. Windows-gated |
