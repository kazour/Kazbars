...
**Current as of:** 2026-08-22 (preview control panel retitled "KazBars Preview", rows grouped under Player Grids / Target Grids / Tools headers with a version-credit footer — `d.PF` now carries `version`, and the core's `modules` registration order is z-order only; `enterPreview` adds the Tools rows by explicit gated `addExtra` calls).
