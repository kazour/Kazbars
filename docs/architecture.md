...
**Current as of:** 2026-07-12 (inventory line counts refreshed for the dead-code removal pass — no import edges, clusters, or flows changed).
...
| `src/kazbars/live_tracker_panel.py` | 508 | Live Tracker Toplevel orchestrator |
...
| `src/kazbars/deeps_meter.py` | 481 | `DeepsMeter` daemon thread — tail loop, log rotation detection, `is_live` probe via `CreateFile` exclusive-share, configurable rolling-window width (`set_window_seconds` recreates the trackers). Publishes `MeterSnapshot` (focus is no longer probed here — the shared `ForegroundWatcher` owns it) |
