...
**Current as of:** 2026-06-10 (**panel-construction smoke test** — new `tests/test_panels_construct.py` boots a real `KazBarsApp` under pytest (userdata relocated to a tmp dir via the `kazbars.userdata.app_path` seam — `userdata.py` binds the name at import, so patching `paths.app_path` would miss; `update_check.check_for_updates` + `content_update.check_and_apply` no-opped; prefs pre-seeded with a `game_path` so the first-launch modal never arms) and opens every satellite panel through its app-side opener — boss timer, Deeps, Damage Numbers, Damage Number Colors, Profile Manager, Buff Display editor, About — closing each via its registered `WM_DELETE_WINDOW` protocol. An AST canary forces every `_open_*`/`_show_*` method on `KazBarsApp` into the tested list or an explicit deferred list, so new panels self-enroll. Catches "panel won't even construct" breakage (bad widget kwarg, renamed token, missing app attribute, broken import wiring) in seconds; event flow/visuals stay `/smoke`. Zero production changes. Prior pass same day — **toast system overhaul** — state-driven rework of the `ui_components.py` toast stack (454 → 587): new pure `ToastModel` (arrival order, coalesce-by-key, visible cap 3 + overflow FIFO queue, two-tier priority — warning/danger jump queued info/success, never displace a visible toast) with `ToastManager` reduced to its Tk renderer. All placement now flows through one `_layout()` authority that measures only after `update_idletasks` and places each frame at model slot + animation offset — closing the three overlap bugs (stale 1-px `winfo_reqheight` read at show-time stacked the old toast onto the new; the entrance animation's captured `target_y` stomped mid-slide relayouts; keyed coalesce grew taller without relayout into the neighbor above). New behaviors, zero call-site changes: `duration=None` (the new `app_toast` default) means per-severity defaults — info/success 4 s, warning 6 s, danger/error 8 s — explicit durations still win; hover pauses the dismiss timer (full restart on leave); click runs `on_click` if set, then dismisses; and `'danger'` now maps to the danger color (previously absent from `STYLES`, silently falling back to accent). Headless guard: new `tests/test_toast_model.py`; renderer/animation behavior stays `/smoke`. Prior pass same day — **`build_loading.py` split** — pure file-move refactor, no behavior change: the 818-line module mixed the build-progress screen with the welcome/About/close-game popups, which share only their visual chrome. The popup family moved to the new `app_popups.py` (383 — shared chrome `make_popup_shell`/`draw_close_button`/`center_popup` now public + the `WIDTH`/`BG`/`BORDER_COLOR`/`SCANLINE_STEP` frame constants + the three popups); `build_loading.py` (465) keeps only `BuildLoadingScreen` and imports the chrome one-way. Consumers rewired: `app.py` (About) + `first_launch` (welcome) now import `app_popups`; `build_action` imports both. The duplicated local `SCANLINE_ALPHA = 12` collapsed onto the existing `ui_helpers` token, same value. Prior pass same day — guard-test hardening: `docs/flows.md` refs are now **function-anchored** — `` `callable()` — file.py``, never `file:line` — with an AST-resolver guard folded into `tests/test_docs_in_sync.py` (subject callables must resolve in the referenced file; `:N` suffixes are banned doc-wide); new `tests/test_mypy_gate.py` pins `[tool.mypy] files` == the Tk-free module set in both directions, and the one missed module — `damageinfo_generator.py` — was added to the gate (clean); new `tests/test_app_contract.py` makes the satellite `app`-first-arg seam a checked contract — every `app.<attr>` access must be defined on `KazBarsApp` — which surfaced the one invented attribute, `content_update`'s `_ota_app_update_notified` once-per-session toast flag, now declared in `KazBarsApp.__init__`'s `# State` block; and new `tests/test_design_tokens.py` enforces the design-token rule — no `#hex` literal outside `ui_helpers.py` (black/white blend anchors + the cluster-owned Live Tracker overlay palette excepted) — with the menu bar's two stragglers (`_DD_BORDER_COLOR`, `_MENU_DISABLED_FG`) hoisted into `TK_COLORS` as `menu_border`/`menu_disabled_fg`, values byte-identical so nothing renders differently. Prior pass 2026-06-06: inventory line-count resync + the Damage Numbers **color editor split to its own Game-menu entry** (`damageinfo_colors_panel.py`, `Damage number Colors…`) with panel polish — human-readable card titles, full master-gate dimming via `create_slider_row`'s new `label_sink`/`label_width` — and the Cast Timer overlay's frame-coherent fix (AS2: `onEnterFrame` driver replaced the free-running `setInterval`, killing the "random big number" spike during lag/interrupts; stub-only, no Python topology change). Prior pass 2026-06-03: **`ui_widgets.py` split** — pure file-move refactor, no behavior change: the 1080-line module was carved into the leaf `ui_widgets.py` (283 — core glue: `blend_alpha`/`add_tooltip`/`app_toast`/`flash_status_bar`/`debounced_callback` + event bindings) plus three new siblings — `ui_headers.py` (197 — `create_dialog_header`/`create_app_header`/`update_app_header_color`/`create_tip_bar`), `ui_forms.py` (424 — fields, `ColorSwatch`/`create_rounded_rect`/`draw_grid_cells` + the shared settings-panel builders: card/status-block/slider-row/toggle-button), `ui_collapsible.py` (232 — `CollapsibleSection`). All three depend only on `ui_widgets` (`blend_alpha`/`add_tooltip`) + `ui_helpers` tokens; `ui_widgets` imports none of them, so it stays the leaf core (no cycles). Prior pass 2026-05-31: **Damage Numbers** feature added — a Game-menu popup (`damageinfo_panel.py`) + offset-bake config (`damageinfo_settings.py`) + MTASC-inject generator (`damageinfo_generator.py`) that ships a lean from-scratch rewrite of AoC's `DamageInfo.swf` under `assets/damageinfo/`; threaded through `build_action`/`build_executor` like the console/cast-timer gates, with a one-time `DamageInfo.swf.kazbars.bak` of the stock file for clean revert. New tests: `test_damageinfo_settings.py`, `test_damageinfo_generator.py`. Prior pass 2026-05-30: Deeps "Alarm & Tints" card redesigned — DPS-out alarm is now a 1000–4000/s slider and the four ΔHP-in tint thresholds collapsed into a Tank/Standard `survival_preset`; refreshed the `deeps_panel.py`/`deeps_settings.py`/`test_deeps_settings.py` inventory rows + counts. Prior pass 2026-05-29: `instructions_panel.py` inventory count refreshed 403 → 512 after six help sections were added; inventory line counts resynced to the tree — the Deeps subtree had drifted furthest; added the `__main__.py`/`__init__.py` entry-point rows that were never listed. Drift is now guarded by `tests/test_docs_in_sync.py` and refreshed via the `/sync-docs` command + `doc-maintainer` agent. Prior pass (2026-05-25): overlay consolidation — both overlays unified onto a shared `overlay_engine.HudOverlay` + `OverlayConfig`; one app-owned `focus_watcher.ForegroundWatcher` replaced the per-cluster focus polls; the foreground probe moved to a pure `foreground.py`; `paths.py` centralizes asset/app-path resolution.)
...
|  3 | `grid_model`, `build_utils`, `build_executor`, `live_tracker_settings`, `paths` | Cluster leaves. `paths` is imported directly by `app.py`, `build_utils`, `deeps_parsers` (everyone else gets paths via the `app` object). |
|  2 | `grids_generator` | |
|  1 | `grids_panel`, `custom_menu_bar`, `profile_io`, `game_folder`, `game_resolution`, `build_action`, `build_loading`, `database_editor`, `instructions_panel`, `first_launch`, `live_tracker_panel`, `grid_dialogs`, `boss_timer`, `timer_overlay`, `combat_monitor`, `update_check`, `settings_backup`, `foreground`, `focus_watcher` | Each consumed by exactly one parent — low blast radius by design. (`foreground` ← `focus_watcher`; `focus_watcher` ← `app.py`; `build_loading` ← `build_action`.) |
...
- **`tests/test_design_tokens.py`** — the design-token rule, enforced: no `#hex` string literal outside `ui_helpers.py`. Pure `#000000`/`#ffffff` are allowed anywhere (they're `blend_alpha` math anchors, not palette decisions); the Live Tracker in-game overlay palette (`live_tracker_settings.COLORS` — PIL render domain, cluster-owned) is allowlisted literal-by-literal so a new color there still forces a conscious decision. Stale allowlist entries fail too, and a canary asserts the scanner still lights up on `ui_helpers.py` itself. PIL `(r,g,b)` tuples and the AS2/XML `0x…` machinery are out of scope.
...
- **`tests/test_toast_model.py`** — pure `ToastModel` core: per-severity duration defaults, coalesce-by-key (live / queued / skips-exiting), visible cap + FIFO promotion, two-tier priority, and non-overlapping slot offsets (the toast-overlap regression).
...
- **`tests/test_panels_construct.py`** — panel-construction smoke: boots one real `KazBarsApp` (module-scoped; userdata relocated to tmp via the `kazbars.userdata.app_path` seam, `update_check`/`content_update` no-opped, prefs pre-seeded with a `game_path` so the first-launch modal never arms) and opens every satellite panel through its app-side opener — boss timer, Deeps, Damage Numbers, Damage Number Colors, Profile Manager, Buff Display editor, About — asserting each constructs, then closing it via its registered `WM_DELETE_WINDOW` protocol (withdraw for the two trackers, destroy for the rest). Booting alone covers `GridsPanel`/`DatabaseEditorTab`/`InstructionsPanel`/menu-bar/header construction. An AST canary keeps the list honest: every `_open_*`/`_show_*` method on `KazBarsApp` must appear in OPENERS or the explicit DEFERRED dict. Catches bad widget kwargs, renamed tokens, missing app attributes, and broken import wiring; event flow, visuals, and drag behavior stay `/smoke`. Windows-gated like `/smoke` + CI.
...
| `src/kazbars/build_loading.py` | 465 | `BuildLoadingScreen` — frameless build progress (animated ring, step dots) → results-summary phases. Imports the popup chrome + frame constants from `app_popups` |
| `src/kazbars/app_popups.py` | 383 | Frameless dark popup family: shared chrome (`make_popup_shell`/`draw_close_button`/`center_popup` + `WIDTH`/`BG`/`BORDER_COLOR`/`SCANLINE_STEP`), `show_welcome_popup`, `show_about_popup` (Discord/GitHub links + animated mini-grid), `show_close_game_required_dialog` |
...
| `tests/test_panels_construct.py` | 141 | Panel-construction smoke — boots a real `KazBarsApp` (tmp userdata, network no-opped, prefs pre-seeded) and opens all 7 satellite panels via their app-side openers, closing each via `WM_DELETE_WINDOW`; AST canary self-enrolls new `_open_*`/`_show_*` openers (or an explicit DEFERRED entry). Windows-gated |
