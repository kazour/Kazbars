...
**Current as of:** 2026-07-29 (Target Inspect Panel — new `inspect`/`inspect_panel` modules + `KazBarsInspect` stub threaded through the build gates; counts refreshed).
...
**AS2 class names are load-bearing.** `base.swf` bootstraps `m_Module = new KazBars(this)`, so the generated classes, the `stubs/KazBars*.as` filenames, and `KazBars_core.as.template` must keep the `KazBars*` names (`KazBars`, `KazBarsData`, `KazBarsConsole`, `KazBarsPreview`, `KazBarsSlot`, `KazBarsCastTimer`, `KazBarsStopwatch`, `KazBarsInspect`) to bind against it. A Python-only rename silently breaks the bind — the old `KzGrids` freeze was only lifted by recompiling `base.fla` in Flash CS6 with the new bootstrap and re-exporting `base.swf`; renaming again needs the same Flash re-export. The console (`KazBarsConsole` / `include_console`), cast-timer (`KazBarsCastTimer` / `cast_config`), stopwatch (`KazBarsStopwatch` / the `stopwatch` prefs dict), and target-inspect (`KazBarsInspect` / the `inspect` prefs dict) stubs compile in only when enabled — gated in `grids_generator.py` so MTASC skips the unused stub class entirely. The stopwatch is a count-up Start/Pause/Reset panel drawn entirely at runtime (device-font TextFields resolving to the Arial faces embedded in `base.swf`, no new symbols): draggable title bar with a live-coordinate readout, collapse-to-title-bar via a − / + button, position + collapsed state persisted in the module config archive (`swx`/`swy`/`swc`) on aoc.exe clients, baked X/Y defaults for `/loadclip` clients. The target inspect panel (`stubs/KazBarsInspect.as`) is a runtime-drawn combat sheet for the current target in the visual language of the game's default inspect window: a 250 ms `GetStat` poll over a 57-id watch list (3 clean warm-up passes before showing; the id 1 + id 54 collapse is treated as logout/zone teardown, not data), subject resolved via `Character.GetCharacter` with a `Dynel.GetDynel` fallback for destructibles (a minimal intrinsic stub added to the `assets/common_stubs/com/GameInterface/Game/` tree alongside `Character`/`CharacterBase`), sheet-exact armor/protection/mitigation/crit/critigation/CDI/bonus-spell-damage synthesis, and a PvP section shown only on player targets — gated on the engine's own `ID32.IsPlayer()` **and** a decoded attribute spread, both of which must agree (the classifier is measured truthful on players but never sampled on mobs, so it vetoes rather than confirms alone). Attribute *presence* is not a discriminator: attributes are x10+10 encoded, so an NPC template carrying them at base still reads raw 10 — which is exactly what put a PvP block on city guards. Every dimension derives from the baked `fontSize`; position and collapse mirror the stopwatch (baked X/Y + `startCollapsed` defaults, name-strip drag with live coordinates and a − / + collapse button, aoc.exe archive keys `inx`/`iny`/`inc`). The watch list, the sheet syntheses and their level-80 constants, the display gates, and the in-game verification checklist are documented in [`inspect-panel.md`](inspect-panel.md) — read that before changing any number in the stub.
...
```
src/kazbars/app.py  → profile_io, profile_manager, game_folder, game_resolution, build_action, buff_display_editor, first_launch, custom_menu_bar, update_check, content_update, settings_backup, stopwatch_panel, inspect_panel
...
| 13 | `grid_model` | Grid dataclasses + the screen-bounds constants (`SCREEN_MAX_X`/`SCREEN_MAX_Y`) — pulled by every grid-editing surface plus the cast-timer/stopwatch/inspect config layers and their dialogs. Pure data, stable surface. |
...
| `src/kazbars/build_loading.py` | 491 | `BuildLoadingScreen` — frameless build progress (animated ring, step dots) → results-summary phases. Imports the popup chrome + frame constants from `app_popups` (incl. `keep_modal_restorable`) |
...
| `src/kazbars/buff_display_editor.py` | 577 | Default Buff Bars dialog (UI). Pure XML helpers in `buff_xml.py`; Customized-file writes go through `safe_write_text` (atomic) |
...
| `src/kazbars/app.py` | 716 | Entry point + `KazBarsApp` root window (widgets, menu, lifecycle); `__init__` calls `ensure_layout()`, points `profiles_path`/`settings_path` at `userdata/`, builds `self.settings = Store(PREFS_SCHEMA, userdata_root())`, loads the buff DB via `database.load_layers(stock, content, user)` (DB editor wired with a `DeltaStore` + `get_floor`), and kicks the OTA check (`content_update.check_and_apply`, deferred to first-launch completion on a fresh install); Updates menu carries the auto-update toggle + check-now/revert; Extras menu carries the In-game stopwatch + Target inspect panel dialogs; File menu carries "Manage profiles…" |
...
| `src/kazbars/ui_forms.py` | 433 | Form fields + shared settings-panel builders: `labeled_spinbox`/`labeled_combobox`/`position_entry`, `draw_grid_cells`, `create_rounded_rect`, `ColorSwatch` (rounded swatch + themed `ColorChooserDialog`), and the group both config panels share — `create_card`, `create_status_block`, `create_slider_row` (optional `value_width` for the readout label, `notch` for a centered default tick on symmetric sliders, `label_width` to align descriptor columns, and `label_sink` so a master gate can grey the row's descriptor + value labels alongside its control), `toggle_button_state`, `create_toggle_action_button`, `refresh_toggle_button`. Imports `add_tooltip` from `ui_widgets` |
| `src/kazbars/ui_collapsible.py` | 232 | `CollapsibleSection` (with `set_dimmed`). Imports `blend_alpha` from `ui_widgets` |
| `src/kazbars/live_tracker_panel.py` | 511 | Live Tracker Toplevel orchestrator |
| `src/kazbars/timer_overlay.py` | 372 | In-game transparent Live Tracker overlay — a `HudOverlay` consumer (`_render_content`: two text rows + cycle-timer dock with 8-direction stroke; `_measure`: font-derived auto-size, no resize handle) |
| `src/kazbars/ui_components.py` | 586 | Toast stack — pure `ToastModel` (cap 3 + queue, two-tier priority, coalesce-by-key) + `ToastManager` Tk renderer (single `_layout()` authority, hover-pause, click-dismiss) — plus `DragReorderManager`, scrollable frame |
| `src/kazbars/grids_generator.py` | 692 | AS2 code generation from grid configs (optional console hooks via `include_console`; optional cast-timer overlay hooks + `d.CAST` block via `cast_config` → `include_cast_timer`; optional stopwatch hooks + `d.SW` block via `stopwatch_config` → `include_stopwatch`; optional inspect-panel hooks + `d.INS` block via `inspect_config` → `include_inspect`). Also holds `CUSTOM_ICON_LINKAGE` (null-icon buff IDs → baked `base.swf` symbol names), emitted into `KazBarsData.CUSTOMICON` |
| `src/kazbars/boss_timer.py` | 399 | Boss timer state + UI |
| `src/kazbars/instructions_panel.py` | 880 | Help/instructions view — two-pane knowledge base: categorized nav + live search over a `SECTIONS` data model, content rendered per selection |
...
| `src/kazbars/combat_monitor.py` | 301 | Combat log parser feeding the tracker (worker loop carries a thread-identity guard so a quick Stop→Start can't leave two workers double-firing) |
...
| `src/kazbars/build_executor.py` | 467 | MTASC compile + deploy; Damage Numbers backup/restore (bundled pristine as the stock source of truth, install via stage-to-temp then back-to-back `os.replace` commit) + `_prepare_textcolors` (surgical, invertible direction flips in the skin's Customized/TextColors.xml — resource-loss + incoming/self, on→−1 / off→1, created from Default/ when absent; colors are never touched here; uninstall flips directions back and leaves colors + `.bak`) |
...
| `src/kazbars/profile_share.py` | 137 | Pure `KZBARS1:` codec — `encode_profile`/`decode_profile` (gzip+base64), `collect_referenced_user_buffs` (the user-provenance buffs a profile references, across int-ID + name forms) + `merge_imported_buffs` (merge into `database_user.json`: skip on an ID collision, rename on a name-only collision), so an export is self-contained. Imports stdlib + `buff_db_layers` only |
...
| `src/kazbars/build_action.py` | 294 | Build & Install flow (compile + install on a worker thread) |
| `src/kazbars/ui_helpers.py` | 195 | Design tokens + `setup_custom_styles` + `style_treeview_heading` |
...
| `tests/test_profile_share.py` | 152 | `KZBARS1` codec — encode/decode round-trip, corrupt/truncated rejection, `collect_referenced_user_buffs` (exactly user-provenance refs across int-ID + name forms, de-duped), self-contained round-trip into an empty DB, import merge (skip on any-ID collision, rename on name collision) |
...
| `src/kazbars/inspect.py` | 50 | Target inspect panel config (pure data): a `settings_core.Schema` (`enabled`/`x`/`y`/`fontSize`/`startCollapsed`) validated via `validate_all`; its `enabled` flag is the `include_inspect` build gate. No Tk |
| `src/kazbars/inspect_panel.py` | 147 | Target Inspect Panel dialog (Extras ▸ Target inspect panel…) — build-gate toggle, baked default X/Y, baked font size (8–48, the whole panel scales with it), start-collapsed flag; single-instance via `app.inspect_dialog`; persists the `inspect` prefs dict |
| `src/kazbars/damageinfo_settings.py` | 483 | Damage Numbers config (pure data; validation + atomic I/O delegate to `settings_core` — `GLOBAL_SETTINGS` doubles as the Schema's source): `GLOBAL_SETTINGS` bake-map (symmetric offset ranges + `invert`/`relative` UI flags for the position sliders + target file + regex), `GAME_DEFAULTS`, `PRESETS` (Default/Performance — carry the animation timing), `SPREAD_SPACING_OPTIONS` (one radio → both zig-zag offsets) + `spread_spacing_option`, the per-source color catalog (`PAIRED_GROUPS`/`SHARED_SOURCES`/`ALL_SOURCE_NAMES`) + `normalize_color` (the color editor's UI catalog — colors themselves are NOT settings), validate/`compute_final_value`/`readout`/`apply_preset`, `is_offset_key`, load/save. No Tk |
...
| `src/kazbars/prefs.py` | 103 | Machine-local prefs: `PREFS_SCHEMA` (strict `settings_core.Schema` for `prefs.json`, with the structured `window_positions` + `buff_display_section_open` + `stopwatch` + `inspect` dict fields, plus the OTA `content_version`/`auto_update_content`). `app.settings` is a `settings_core.Store` on this schema; the `get_setting`/`set_setting` proxy + ~20 `app.settings` call sites use its `get`/`set`/`save()`/`reload()`/`data` surface directly. Pure, no Tk |
| `src/kazbars/update_check.py` | 114 | GitHub release check — silent launch check + Updates-menu manual check (`check_for_updates_manual`, always answers with a toast) over the shared `fetch_latest` (also used by the About popup); named main-thread dispatchers; `_parts` keeps each dot-component's leading digits so suffixed tags (`2.3.0-rc1`) still compare |
| `tests/test_update_check.py` | 35 | `_parts` version compare — plain semver, numeric (not lexicographic) ordering, suffixed tags keep their numeric prefix (`2.3.0-rc1` → `(2, 3, 0)`, never falsely up-to-date), garbage tag → `()` conservative |
...
| `src/kazbars/paths.py` | 46 | Path constants: `PACKAGE_ROOT`/`ASSETS`/`KAZBARS_ASSETS` (bundled read-only assets, dev + frozen) + `app_path()` (user-writable runtime root next to the .exe) |
...
| `tests/test_panels_construct.py` | 247 | Panel-construction smoke — boots a real `KazBarsApp` (tmp userdata, network no-opped, prefs pre-seeded) and opens all 9 satellite panels via their app-side openers, closing each via `WM_DELETE_WINDOW`; AST canary self-enrolls new `_open_*`/`_show_*` openers (or an explicit DEFERRED entry). Also hosts the ui_widgets binding-hygiene tests (idempotent `add_tooltip`, hover-colors add="+") — they must ride this module's Tk instance, since a second Tcl interp won't initialize on the hosted CI runner. Windows-gated |
| `tests/test_grids_generator.py` | 313 | `CodeGenerator.include_console`, `include_cast_timer` (via `cast_config`), `include_stopwatch` (via `stopwatch_config`), AND `include_inspect` (via `inspect_config`) on/off output checks |
...
| `tests/test_inspect.py` | 64 | `inspect` config defaults (off by default), X/Y + fontSize clamping, start-collapsed coercion, bad-value fallback, sanitized shape |
...
| `tests/test_combat_monitor.py` | 150 | `_process_line` dispatch, player extraction, latest-log discovery, start-without-folder guard, quick-restart stale-worker regression (thread-identity guard) |
...
| `tests/test_build_compile.py` | 137 | MTASC compile-integration — whole codegen → bundled `mtasc.exe` exit-0 (escaping end-to-end + console/cast/stopwatch/inspect variants + all-features-together); the only check bridging Python-side correctness to the SWF side; win32 + compiler gated |
...
| `tests/test_deeps_meter.py` | 469 | 40 cases — file selection, lifecycle (incl. the quick-restart stale-worker regression), `_process_line` dispatch, configurable-window reset, attach-at-EOF vs boundary resume, held-open-file end-to-end |
