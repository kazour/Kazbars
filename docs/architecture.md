...
**Current as of:** 2026-08-17 (the Stopwatch, Inspect Panel and Cast Timer dialogs now follow the Damage Numbers layout conventions — tip bar, master gate row above `create_card` cards, `BTN_DIALOG` footer, full gated greying — with `labeled_spinbox` gaining `label_sink` and `ColorSwatch` gaining `set_enabled` in `ui_forms` to support it).
